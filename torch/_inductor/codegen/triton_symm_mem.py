    ...
    for i in range(ws):
        ...
        with load_buffer.indent():
            load_buffer.writeline(
                ...
                f"tl.load(symm_signal_pad_ptrs + {i}) + SYMM_RANK * 8, "
                f"_nvshmem_one_i64, 5, {i})"
            )
    ...
    for i in range(ws):
        ...
        with load_buffer.indent():
            load_buffer.writeline(
                ...
                f"tl.load(symm_signal_pad_ptrs + SYMM_RANK) + {i * 8}, "
                f"3, _nvshmem_epoch)"
            )
...
def codegen_nvshmem_prologue(kernel, code):
    ...
    code.splice(
        ...
        _symm_cols = tl.arange(0, R0_BLOCK)
        _symm_col_mask = _symm_cols < r0_numel
        _nvshmem_one_i64 = tl.load(symm_signal_pad_ptrs + 0) * 0 + 1
        """
    )
...
def codegen_nvshmem_epilogue(kernel, code):
    ...
    for i in range(ws):
        ...
        with code.indent():
            code.writeline(
                f"_nvshmem_signal_op("
                f"tl.load(symm_signal_pad_ptrs + {i}) "
                f"+ (SYMM_WORLD_SIZE + SYMM_RANK) * 8, "
                f"_nvshmem_one_i64, 5, {i})"
            )
    for i in range(ws):
        code.writeline(f"if {i} != SYMM_RANK:")
        with code.indent():
            code.writeline(
                f"_nvshmem_signal_wait_until("
                f"tl.load(symm_signal_pad_ptrs + SYMM_RANK) "
                f"+ (SYMM_WORLD_SIZE + {i}) * 8, "
                f"3, _nvshmem_epoch)"
            )
